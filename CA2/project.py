...
def show_checkout():
    ...
    r_p_submit = tk.Button(menu_frame, text="Add", command=partial(addOrder,r_p_combo,r_p_qty_entry,cart,prices))
    ...
    p_submit = tk.Button(menu_frame, text="Add", command=partial(addOrder,p_combo,p_qty_entry,cart,prices))
    ...
    b_submit = tk.Button(menu_frame, text="Add", command=partial(addOrder,b_combo,b_qty_entry,cart,prices))
    ...
    s_submit = tk.Button(menu_frame, text="Add", command=partial(addOrder,s_combo,s_qty_entry,cart,prices))
    ...
    ss_submit = tk.Button(menu_frame, text="Add", command=partial(addOrder,ss_combo,ss_qty_entry,cart,prices))
...
# Header
canvas.create_text(375, 50, font="times 28 bold italic underline", text="MENU", fill="white")

# Rice/Pasta
canvas.create_text(150, 90, font="times 18 bold underline", text="RICE/PASTA", fill="black", justify="center")

rp_dict = {"Jollof Rice": 350, "Coconut Fried Rice": 350, 
                   "Jollof Spaghetti": 350}
l_start = 120
for k,v in rp_dict.items():
    canvas.create_text(57, l_start, font="times 14 bold", text=f"{k}", fill="black", anchor='w')
    canvas.create_text(300, l_start, font="times 14 bold", text=f"{v}", fill="black", anchor='w')
    l_start += 20
    rp_dishes.append(k)

# Proteins
canvas.create_text(550, 90, font="times 18 bold underline", text="PROTEINS", fill="white", justify="left")

p_dict = {"Sweet Chili Chicken": 1100, "Grilled Chicken Wings": 400,
          "Fried Beef": 400, "Fried Fish": 500, "Boiled Egg": 200,
          "Sautered Sausages": 200}
r_start = 120
for k,v in p_dict.items():
    canvas.create_text(450, r_start, font="times 14 bold", text=f"{k}", fill="white", anchor='w')
    canvas.create_text(680, r_start, font="times 14 bold", text=f"{v}", fill="white", anchor='e')
    r_start += 20
    p_dishes.append(k)

# Beverages
canvas.create_text(550, 290, font="times 18 bold underline", text="BEVERAGES", fill="white", justify="center")
b_dict = {"Water": 200, "Glass Drink (35cl)": 150, "PET Drink (35cl)": 300,
          "PET Drink (50cl)": 350, "Glass/Canned Malt": 500, "Fresh Yo": 600,
          "Pineapple Juice": 350, "Mango Juice": 350, "Zobo Drink": 350}
r_start = 320
...
# Side dishes
canvas.create_text(150, 230, font="times 18 bold underline", text="SIDE DISHES", fill="black", justify="center")

s_dict = {"Savoury Beans": 350, "Roasted Sweet Potatoes": 300,
          "Fried Plantains": 150, "Mixed Vegetable Salad": 150,
          "Boiled Yam": 150}
l_start = 260
for k,v in s_dict.items():
    canvas.create_text(57, l_start, font="times 14 bold", text=f"{k}", fill="black", anchor='w')
    canvas.create_text(300, l_start, font="times 14 bold", text=f"{v}", fill="white", anchor='w')
    l_start += 20
    s_dishes.append(k)

# Soups and Swallows
canvas.create_text(190, 400, font="times 18 bold underline", text="SOUPS & SWALLOWS", fill="white", justify="center")

ss_dict = {"Eba": 100, "Pounded Yam": 100, "Semovita": 100,
           "Atama Soup": 450, "Egusi Soup": 400}
l_start = 430
for k,v in ss_dict.items():
    canvas.create_text(57, l_start, font="times 14 bold", text=f"{k}", fill="white", anchor='w')
    canvas.create_text(300, l_start, font="times 14 bold", text=f"{v}", fill="white", anchor='w')
    l_start += 20
    ss_dishes.append(k)
...
order_button = tk.Button(canvas, text="Order Now", padx=6, pady=5, command=show_checkout)
